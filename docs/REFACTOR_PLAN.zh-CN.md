...
**G3 菜单协议定案关闭，核心代码重构 100% 完成**：基于 `GenericStack` 的受管局部展示槽包装（`LogicalMenuSlot` + `LogicalMenuInteraction` + `MixinLogicalMenu`）定案保留，避免引入自定义网络包或侵入性全局补丁；客户端全附属冒烟测试已通过 1,000,000 大堆叠无损解包渲染与流体槽 16,000 mB 状态验证；服务端通过 78 项 GameTest，覆盖大堆叠外部写入、`broadcastChanges` 同步、取物守恒及关闭重开一致性（`d19a198`）。至此 G1（配方投影与 tick 语义）、G2（生命周期安全）、G3（菜单与局部协议）、G4（兼容边界收口）四大门槛全部关闭，核心代码重构正式达成 100% 完成，全面转入发布候选阶段。

**发布候选配方扩展推进中**：ExtendedAE 切片器的 `calculation`、`engineering`、`silicon` 已补真实共享批处理守恒覆盖；AE2CS 四台机器的抽样配方（熵变 COOL 模式/流体双向、粉碎机赛特斯石英粉、聚合器福鲁伊克斯水晶 4 倍产物、蚀刻器计算处理器）已全部补齐真实守恒与状态机覆盖（`396955d`、`a63f400`）。`ae2cs` 与 `all` 严格离线 build、78 项 GameTest 全部通过。完整进度与剩余边界继续以进度文档为准。

**资源封存包 UI 定案**：仅支持物品，使用内部物品自身图标（保留 NBT 模型和颜色），不兼容旧流体包，不提供流体兜底图标。数量继续以 long 保存，渲染只使用数量为 1 的合法 ItemStack；服务端取出与回插仍为权威路径。

**用户定案：破坏机器时流体不掉落。** 罐内流体、未完成批次预留流体、已完成待输出流体均丢弃，不生成封存包；仅破坏操作适用，正常加工、降容、卸载和保存仍要求流体守恒。本条优先于下文原始设计中的破坏全资源无损要求。四台 AE2CS 混合物品真实破坏已补齐；all 离线 build 和 78 项 GameTest 通过。

本文件保留实施前审计与目标设计；当前状态以 [重构进度](REFACTOR_PROGRESS.zh-CN.md) 为准。四大完成门槛（G1~G4）已全部关闭，代码重构已 100% 闭环。
