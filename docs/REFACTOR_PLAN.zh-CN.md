...
**发布候选配方扩展推进中**：ExtendedAE 切片器的 `calculation`、`engineering`、`silicon` 已补真实共享批处理守恒覆盖；AE2CS 四台机器的抽样配方（熵变 COOL 模式/流体双向、粉碎机赛特斯石英粉、聚合器福鲁伊克斯水晶 4 倍产物、蚀刻器计算处理器）已全部补齐真实守恒与状态机覆盖（`396955d`、`a63f400`）。`ae2cs` 与 `all` 严格离线 build、77 项 GameTest 全部通过。完整进度与剩余边界继续以进度文档为准。
...
本文件保留实施前审计与目标设计；当前状态以 [重构进度](REFACTOR_PROGRESS.zh-CN.md) 为准。生命周期安全门槛已经关闭：AE2CS 混合物品真实破坏、真实区块卸载/重载和资源包拾取、分批解包、回插均已有自动证据。代码重构完成门槛仅余真实客户端世界菜单验收与最终局部协议定案。
