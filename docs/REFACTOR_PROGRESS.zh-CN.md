...
`3907c43` 后的两轮审查共提出 R1–R7，当前均已修复。R5 的分页状态、隐藏槽点击和 Shift 入机隔离已纳入正式测试，并在修复后重跑 96 项 GameTest 与集成客户端；R6/R7 的当次 JSON 与当前 Jar 选择规则已有独立负向夹具。最终性能基准、旧世界迁移和生产发布沙盒仍需对修复后最终产物重新取证，不能仅沿用历史报告。
...
| G3 菜单与交互 | 代表路径关闭 | GenericStack 局部包装保留；R5 修复后，四页各仅 4 槽启用，隐藏页 PICKUP/QUICK_MOVE 与 Shift 入机隔离通过正式测试；集成客户端经真实翻页包验证隐藏页拒绝取物、切回与账本一致，原有重开、外部更新、断线重连和多机显示链路同时通过 |
| G4 兼容与构建 | 实现关闭，发布证据待刷新 | 五组合有历史证据；依赖锁、206 项静态审计和发布裁剪接入 build/check；R7 已按当前项目版本精确选包并核对运行版本，多版本/缺失当前 Jar 负向测试通过。修复后生产 Dedicated Server 沙盒尚未重跑 |
| 正式发布候选（RC） | 待复核 | R1–R7 均已修复，96 项 `all` GameTest、受影响客户端路径和严格构建通过；最终基准、迁移、生产发布沙盒及完整五组合仍待刷新 |
...
- **R5 / ExtendedAE 菜单分页：** `LogicalMenuSlot.setSlotEnabled` 同步包装槽与原槽，`mayPlace`、`mayPickup`、`remove` 和 `safeInsert` 均受包装槽启用状态约束。新增三项正式 GameTest，覆盖第 0–3 页每页仅 4 槽启用、隐藏页 PICKUP/QUICK_MOVE 无转移、玩家 Shift 入机不进入隐藏泳道；全附属 96 项通过。集成客户端改用 ExtendedAE 真实翻页包，验证第 4 页点击第 1 页输出无效，切回后 1,000,000 数量不变，再正常取出 64；原有多机与断线重连链路一并通过。
- **R6 / 基准证据：** 仅从当次 GUID 沙盒读取 JSON，要求可解析、`success=true`、`totalScenarios=8` 且场景数组为 8 项后才发布到公共报告；Java 写文件异常改为使运行失败。
- **R7 / 发布选包：** 根据 `gradle.properties` 的当前 `mod_id/mod_version` 精确定位 Jar，不再扫描后取首个文件；生产服务器报告版本必须等于当前声明版本，摘要记录审查提交与 SHA-256。
- 新增 PowerShell 负向回归，确认当次 JSON 缺失/损坏时即使有历史公共报告也失败，多版本 Jar 共存时选择当前版本，当前 Jar 缺失时失败。
- 修复后验证：`all` 严格离线 build 通过（10,000 组核心契约、1,000 组故障恢复、206 项 Mixin 审计、发布裁剪）；`all` 与 `none` 均为 96 项 GameTest 通过；`all` 集成客户端交互脚本通过。基准与生产发布脚本因其 EULA 参数要求显式操作员确认，本轮未擅自代为确认或执行。
...
当前注册 **96 项 GameTest**。没有安装对应附属时，相关测试主动跳过，因此 `none` 显示 96 项通过不代表执行了全部附属功能；必须结合启用日志和 `all` 结果。
...
| 优先级 | 工作 | 完成标准 |
|---|---|---|
| 代表路径已完成 | 联机客户端菜单与机器显示 | long/NBT、取放/Shift、外部更新、关闭重开、切片器与 4 泳道分页、隐藏页拒绝交互、流体 Tooltip、四机同台 3D 浮空产物显示、客户端断线重连账本守恒已通过；远端高延迟/丢包不在该 loopback 证据范围内 |
| 已完成 | 真实旧世界迁移 | 旧格式、超容、处理中机器的迁移前后账本；已在真实 Anvil 物理副本完成四阶段验证并升轨规范格式 |
| 实现已修，待刷新 | 基础性能测量 | 历史修正后基准为首批 5 ticks、最高 3768 items/t；R6 已绑定当次 8 场景 JSON，最终发布前重跑 |
| 实现已修，待刷新 | 可重复构建与发布包 | 206 项 Mixin 审计与零测试类泄漏严格构建通过；R7 已绑定当前 `mod_id/mod_version` Jar 并校验服务器报告版本，最终 Dedicated Server 沙盒待重跑 |
