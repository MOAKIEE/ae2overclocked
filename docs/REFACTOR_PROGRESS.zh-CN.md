...
| G1 配方与 tick 行为 | 部分关闭 | 配方投影与代表自然调度已有证据；四类机器进度已同步；AE2 压印器压合脉冲和切片器渲染产物的包往返已修复。真实客户端可见效果及 ExtendedAE 压印器动画仍待验收 |
...
**保留边界：** GameTest 用第二个方块实体执行真实更新流解码，但不是联网客户端；客户端冒烟确认渲染注入可应用，却未在世界内肉眼捕获压合和切片器三维动画。ExtendedAE 四泳道压印器的全局压合动画也尚未接入。因此这批关闭已知同步缺口，不关闭完整客户端验收。
...
| 菜单与显示 | 受管交互、包回调/编解码、关闭归还、客户端图标冒烟；AE2 压印器与切片器显示更新流往返 | 真实联机预测、世界内动画观察、ExtendedAE 压印器动画、超容提示、机器专属流体 GUI |
...
当前注册 **84 项 GameTest**。没有安装对应附属时，相关测试主动跳过，因此 `none` 显示 84 项通过不代表执行了全部附属功能；必须结合启用日志和 `all` 结果。
...
| 优先级 | 工作 | 完成标准 |
|---|---|---|
| 发布前 | 联机客户端菜单与机器显示 | long/NBT、取放/Shift、外部更新、关闭重开/重连、超容/流体提示；肉眼确认 AE2 压印器和切片器显示，补 ExtendedAE 压印器动画或记录取舍 |
